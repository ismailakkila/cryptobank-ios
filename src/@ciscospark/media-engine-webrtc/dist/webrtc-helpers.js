...
function getMediaDirectionFromTracks(kind, pc) {
  ...
  var senders = pc.getLocalStreams().filter(function(mediaStream) {
  ...
  var senderTracks = [];
  senders.forEach(function(mediaStream) {
    mediaStream.getTracks().forEach(function(track) {
      senderTracks.push(track);
    });
  });

  var send = senders.length > 0 && senderTracks.reduce(function (acc, track) {
    return acc || track.enabled;
  }, false);

  var receivers = pc.getRemoteStreams().filter(function(mediaStream) {
    var tracks = mediaStream.getTracks().filter(function(track) {
      return track && track.kind === kind;
    });
    if (tracks.length > 0) {
  ...
  var receiverTracks = [];
  receivers.forEach(function(mediaStream) {
    mediaStream.getTracks().forEach(function(track) {
      receiverTracks.push(track);
    });
  });

  var recv = receivers.length > 0 && receiverTracks.reduce(function (acc, track) {
    return acc || track.enabled;
  }, false);
