...
var WebRTCMediaEngine = (_dec = (0, _common.whileInFlight)('gumming'), _dec2 = (0, _lodashDecorators.debounce)(500), (_class = function () {
  (0, _createClass3.default)(WebRTCMediaEngine, [{
    ...
    get: function get() {
      ...
      */
      return this.pc.getLocalStreams().reduce(function(acc, m) {
    ...
    get: function get() {
      ...
      */
      return this.pc.getRemoteStreams().reduce(function(acc, m) {
        m.getTracks().forEach(function(t) {
          if (t) {
            acc.push(t);
          }
    ...
    value: function getUserMedia(constraints) {
      ...
      return mediaDevices.enumerateDevices().then(function(sourceInfos) {
        if (finalConstraints.video) {
          var videoSource = sourceInfos.filter(function(sourceInfo) {
            return (sourceInfo.kind === "videoinput" && sourceInfo.label === "Front Camera");
          });

          var deviceIdParams = [];
          if (videoSource.length === 1) {
            deviceIdParams = [{sourceId: videoSource[0].deviceId}];
          }
          ...
          finalConstraints = Object.assign({}, finalConstraints, videoConstraints);
        }
        return mediaDevices.getUserMedia(finalConstraints);
      });
    }
    /**
  ...
  (0, _createClass3.default)(WebRTCMediaEngine, [{
    ...
    value: function _prepareIceGatherer() {
      ...
      if (needsIce) {
        _this2.pc.createOffer(_this2.offerOptions)
          .then(function(offer) {
            _this2.pc.setLocalDescription(offer);
          });
    ...
    value: function _setNewMediaDirection(kind, direction) {
      ...
      if (isSending) {
        var senders = this.pc.getLocalStreams().find(function(mediaStream) {
          return mediaStream && mediaStream.getTracks().find(function(t) {
            return t && t.kind === track.kind;
          });
        });
        ...
        if (senders.length > 0) {
          ...
          senders.forEach(function (m) {
            m.getTracks().forEach(function(t) {
              t.enabled = localTracks.includes(t);
            });
          });
    ...
    value: function setMedia(kind, direction, trackOrConstraint) {
      ...
      return new _promise2.default(function (resolve) {
        ...
        var receivers = _this7.pc.getRemoteStreams().filter(function(mediaStream) {
    ...
    value: function _getUserMedia() {
      ...
      return new _promise2.default(function (resolve) {
        ...
        if (_this8.constraints.audio === true && _this8.pc.getLocalStreams().find(function (mediaStream) {
          return mediaStream && mediaStream.getTracks().find(function(track) {
        ...
        if (userStream) {
          ...
          userStream.getTracks().forEach(function (t) {
            return p.push(_this8.addOrReplaceTrack(t, userStream.id));
          });
        }
    ...
    value: function addOrReplaceTrack(track, id=null) {
      var _this9 = this;

      var p = [];
      if (!this.localMediaStream && id) {
        this.localMediaStream = new MediaStream();
        this.localMediaStream.id = id;
        this.localMediaStream.reactTag = id;
      }
      var sender = this.pc.getLocalStreams().find(function(mediaStream) {
        return mediaStream && mediaStream.getTracks().find(function(t) {
          return t && t.kind === track.kind;
        });
      });
        ...
        if (sender.track === track) {
          ...
          p.push(sender.replaceTrack(track).then(function () {
            ...
            _this9.logger.info('adding as new ' + track.kind + ' track to peerConnection');
             track.streamReactTag = _this9.localMediaStream.reactTag;
            _this9.pc.removeStream(_this9.localMediaStream);
            _this9.pc.addStream(_this9.localMediaStream);
          }));
        }
      } else {
    ...
    value: function pauseSendingMedia(kind) {
      ...
      var senders = this.pc.getLocalStreams().filter(function(m) {
        return m.getTracks().filter(function(t) {
      ...
      ['audio', 'video'].forEach(function (kind) {
        ...
        if (direction !== targetDirection) {
          ...
          if (direction.includes('send') !== shouldSend) {
            ...
            _this11.pc.getRemoteStreams().forEach(function(m) {
              m.getTracks().forEach(function(t) {
                if (t && t.kind === kind) {
    ...
    value: function pauseReceivingMedia(kind) {
      ...
      var receivers = this.pc.getRemoteStreams().filter(function (m) {
        return m.getTracks().filter(function(t) {
          return t && t.kind === kind;
        });
      });
    ...
    value: function stop() {
      if (this.pc.signalingState !== 'closed') {
        this.pc.getLocalStreams().forEach(function(m) {
          return m.getTracks().forEach(function(t) {
            return t && t.stop();
          });
        });
        this.pc.close();
